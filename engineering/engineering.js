...
(function () {
  ...
  canvas.width  = 1280;
  canvas.height = 720;
  // Fill black immediately — never shows white
  ...
  function draw(p) {
    ...
    if (imgA && ready[idxA] === 1) {
      ctx.filter = filt;
      ctx.globalAlpha = 1;
      ctx.drawImage(imgA, 0, 0, 1280, 720);
    }
    ...
    if (blend > 0.01 && imgB && ready[idxB] === 1 && idxB !== idxA) {
      ...
      ctx.drawImage(imgB, 0, 0, 1280, 720);
      ctx.globalAlpha = 1;
    }

    ctx.filter = 'none';

    // Dark overlay: hides the white studio background,
    // creates the cinematic dark-reveal without X-ray effect
  ...
  function updateOverlays(p) {
    ...
    if (p < 0.12) {
      ...
      set(glow,   'opacity',   '0');
      set(type,   'opacity',   '0'); set(type, 'transform', 'translateY(-38%) translateY(30px)'); set(type, 'pointerEvents', 'none');
    ...
    // ── 75–90%  Headlights + type emerges
      ...
      set(type,   'transform', `translateY(-38%) translateY(${f(lerp(24, 0, tyT))}px)`);
    ...
    // ── 90–100%  Full reveal
      ...
      set(type,   'transform', 'translateY(-38%)');
